        ...
        _handleMessage(msg) {
            ...
            if (type === '_ws_presence') {
                ...
                if (user) {
                    ...
                    // Deduplication: Handle multiple connections with same username (e.g. Guest, Same User)
                    const isTaken = (name) => Object.values(this.peers).some(p => p.id !== clientId && p.username === name);
                    ...
                    this.peers[clientId] = {
                        ...
                        // Keep original username for avatar lookup to ensure PFP hot-swapping works
                        avatarUrl: sanitizeAvatar(user.avatar_url, originalUsername)
                    };
                }
